                ...
                StreamBuilder(
                    ...
                    builder: (context, AsyncSnapshot<num> snapshot) {
                      return InputDeposit(
                        ...
                        increaseValue: (snapshot.hasData && depositBloc.depositInput.numberValue < snapshot.data)
                            ? depositBloc.addValue
                            : null,
                        ...
                        increaseLongPress: () {
                          timer = new Timer(const Duration(milliseconds: 10), () {
                            timer = new Timer.periodic(const Duration(milliseconds: 250), (i) {
                              if (snapshot.hasData && depositBloc.depositInput.numberValue < snapshot.data != null)
                                depositBloc.addValue();
                            });
                          });
                        },
                                          ...
                                          child: ListTileDefault(
                                            leading: Row(
                                              children: <Widget>[
                                                ...
                                                SizedBox(width: 5),
                                                Text(formatMoney.format(depositBloc.depositInput.numberValue, true),
                                                    style: TextStyle(fontSize: 16)),
                                              ],
                                            ),
                                            child: Text("${(snapshot.data[i].rate / 100).toStringAsFixed(2)}%",
                                                style: TextStyle(fontSize: 16)),
                                            trailing: Text(formatMoney.format(snapshot.data[i].interest, true, true),
                                                style: TextStyle(
                                            ...
                                            isDefault: true,
                                            type: 1,
                                            lefts: snapshot.data[i].isNew == false
                                                ? snapshot.data[i].expiryDate
                                                    .difference(depositBloc.businessDate)
                                                    .inDays
                                                : null,
                                            ...
                                            onTap: () async {
                                              await Navigator.of(context).push(MaterialPageRoute(
                                                  builder: (BuildContext context) => DetailPurchasePage(
                                                      snapshot.data[i], depositBloc.depositInput.numberValue)));
                                              var data = await sessions.flashMessage("purchased");
                                              if (data != null) {
                                                dialogs.alertWithIcon(context,
                                                    icon: Icons.check_circle_outline,
                                                    title: "Success!",
                                                    message:
                                                        "Deposit successful. See My Money to watch your deposit grow!");
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        Text("Show Community Offers",
                            style: TextStyle(
                                fontSize: 15, color: Theme.of(context).primaryColor, fontWeight: FontWeight.w400)),
